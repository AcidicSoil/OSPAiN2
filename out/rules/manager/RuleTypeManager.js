...
exports.RuleTypeManager = void 0;
const types_1 = require("../types");
const RuleValidator_1 = require("../validation/RuleValidator");
const fs = __importStar(require("fs/promises"));
const path = __importStar(require("path"));
class RuleTypeManager {
    constructor() {
        this.monitoringStartTime = null;
        this.processedRules = new Set();
        this.validator = new RuleValidator_1.RuleValidator();
    }
    /**
     ...
     */
    async updateRuleType(filePath, newType) {
        try {
     ...
     */
    async applyBulkUpdates(updates) {
        const result = {
            success: true,
            updatedFiles: [],
            failedFiles: [],
     ...
     */
    async listRuleFiles(directory, recursive = true) {
        try {
     ...
     */
    async findMdcFiles(directory, recursive = true) {
        try {
            const files = [];
     ...
     */
    async readRuleContent(filePath) {
        try {
     ...
     */
    async parseRuleMetadata(content) {
     ...
     */
    determineCurrentType(filePath) {
        // Path-based determination
        if (filePath.includes('/memory/')) {
            return types_1.RuleType.AGENT_REQUESTED;
        }
        else if (filePath.includes('/rules/core/')) {
            return types_1.RuleType.AUTO_APPLIED;
        }
        else if (filePath.includes('/rules/conditional/')) {
            return types_1.RuleType.CONDITIONAL;
        }
        else if (filePath.includes('/user/custom/')) {
            return types_1.RuleType.MANUAL;
        }
        // Default to manual
        return types_1.RuleType.MANUAL;
    }
     ...
     */
    analyzeSuggestedType(content, filePath) {
    ...
    async applyUpdate(update) {
        try {
            ...
            if (metadataMatch) {
                ...
                updatedContent = content.replace(/---\s+([\s\S]*?)\s+---/, `---\n${metadataStr}\n---`);
            }
            else {
...
exports.RuleTypeManager = RuleTypeManager;
//# sourceMappingURL=RuleTypeManager.js.map