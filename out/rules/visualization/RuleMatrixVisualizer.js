...
exports.RuleMatrixVisualizer = void 0;
const KnowledgeGraph_1 = require("../../knowledge/KnowledgeGraph");
const fs = __importStar(require("fs"));
const path = __importStar(require("path"));
const KnowledgeGraphIntegration_1 = require("./KnowledgeGraphIntegration");
class RuleMatrixVisualizer {
    constructor(knowledgeGraph) {
        this.knowledgeGraph = knowledgeGraph || new KnowledgeGraph_1.KnowledgeGraph();
        this.knowledgeGraphIntegration = new KnowledgeGraphIntegration_1.KnowledgeGraphIntegration(this.knowledgeGraph);
    }
    /**
     * Generate visualization data for rule relationships
     */
    async generateVisualizationData(rules, relationships, options = {}) {
     ...
     */
    async generateHtmlVisualization(rules, relationships, outputPath, options = {}) {
     ...
     */
    async generateJsonData(rules, relationships, outputPath, options = {}) {
        const data = await this.generateVisualizationData(rules, relationships, options);
        // Ensure directory exists
        const dir = path.dirname(outputPath);
        if (!fs.existsSync(dir)) {
    ...
    filterRules(rules, options) {
        ...
        if (options.includeTypes && options.includeTypes.length > 0) {
            filtered = filtered.filter(rule => options.includeTypes.includes(rule.currentType));
        }
        // Filter by exclude types
        if (options.excludeTypes && options.excludeTypes.length > 0) {
            filtered = filtered.filter(rule => !options.excludeTypes.includes(rule.currentType));
        }
        return filtered;
    }
    /**
     ...
     */
    filterRelationships(relationships, rules, options) {
        const rulePaths = rules.map(r => r.path);
        let filtered = relationships.filter(rel => rulePaths.includes(rel.sourceRule) && rulePaths.includes(rel.targetRule));
        // Filter by minimum relationship strength
        if (options.minRelationshipStrength !== undefined) {
            filtered = filtered.filter(rel => rel.strength >= options.minRelationshipStrength);
        }
        return filtered;
    }
    /**
     ...
     */
    async enhanceWithKnowledgeGraph(nodes, links, rules, options) {
     ...
     */
    filterNodesByTheme(nodes, themes) {
        const nodeIndices = [];
     ...
     */
    filterNodesByContentType(nodes, contentTypes) {
        const nodeIndices = [];
        // Identify nodes to remove
        nodes.forEach((node, index) => {
     ...
     */
    filterLinksBySemanticSimilarity(links, minSimilarity) {
        const linkIndices = [];
     ...
     */
    applySemanticGrouping(nodes, groupType) {
        nodes.forEach(node => {
     ...
     */
    async generateVisualizationDataWithSemanticAnalysis(rules, relationships, options = {}) {
     ...
     */
    async generateSemanticHtmlVisualization(rules, relationships, outputPath, options = {}) {
        // Generate visualization data with semantic analysis
        const data = await this.generateVisualizationDataWithSemanticAnalysis(rules, relationships, options);
     ...
     */
    createSemanticHtmlTemplate(data, options) {
...
exports.RuleMatrixVisualizer = RuleMatrixVisualizer;
//# sourceMappingURL=RuleMatrixVisualizer.js.map