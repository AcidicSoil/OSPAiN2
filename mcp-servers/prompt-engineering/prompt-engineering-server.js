...
exports.PromptEngineeringServer = void 0;
const axios_1 = __importDefault(require("axios"));
const mcp_server_1 = require("../shared/mcp-server");
const mode_aware_1 = require("../utils/mode-aware");
// Default templates
const DEFAULT_TEMPLATES = {
    default: "Summarize the following text concisely:\n\n{{text}}",
    technical: "Provide a technical summary of the following content, focusing on key components, technologies, and processes:\n\n{{text}}",
    executive: "Create an executive summary highlighting key business points, decisions, and outcomes from the following:\n\n{{text}}",
    bullet: "Extract the key points from the following text as a bullet-point list:\n\n{{text}}",
...
const MODE_TEMPLATES = {
    design: {
        ...
        accessibility: "Summarize accessibility considerations in the following text:\n\n{{text}}",
        component: "Identify key components and their relationships in the following design text:\n\n{{text}}",
        visual: "Summarize visual design elements mentioned in the following text:\n\n{{text}}",
    },
    engineering: {
        architecture: "Extract the architectural patterns and decisions from the following text:\n\n{{text}}",
        implementation: "Summarize the implementation details and code patterns from the following:\n\n{{text}}",
        api: "Extract API details, endpoints, and functionalities from the following text:\n\n{{text}}",
        performance: "Identify performance considerations and optimizations in the following text:\n\n{{text}}",
        security: "Summarize security aspects and considerations from the following text:\n\n{{text}}",
    },
    testing: {
        testcase: "Transform the following text into potential test cases:\n\n{{text}}",
        coverage: "Identify what needs to be tested based on the following text:\n\n{{text}}",
        scenario: "Extract testing scenarios and edge cases from the following text:\n\n{{text}}",
        validation: "Summarize validation requirements from the following text:\n\n{{text}}",
        regression: "Identify potential regression issues from the following text:\n\n{{text}}",
    },
    deployment: {
        release: "Create a release note summary from the following text:\n\n{{text}}",
        configuration: "Extract configuration requirements from the following text:\n\n{{text}}",
        migration: "Identify migration steps or considerations from the following text:\n\n{{text}}",
        infrastructure: "Summarize infrastructure requirements from the following text:\n\n{{text}}",
        monitoring: "Extract monitoring and alerting considerations from the following text:\n\n{{text}}",
    },
    maintenance: {
        issue: "Summarize the issue description and potential causes from the following text:\n\n{{text}}",
        debug: "Extract debugging steps and information from the following text:\n\n{{text}}",
        improvement: "Identify potential improvements or optimizations from the following text:\n\n{{text}}",
        support: "Create a customer support response based on the following text:\n\n{{text}}",
        documentation: "Summarize documentation needs or changes from the following text:\n\n{{text}}",
    },
};
/**
 * Prompt Engineering Assistant Server
 */
class PromptEngineeringServer extends mcp_server_1.MCPServer {
    constructor(config) {
        super(config);
        this.cachingStrategy = "default";
        this.templates = {};
        this.ollamaEndpoint =
        ...
        this.modeService = new mode_aware_1.ModeAwareService("prompt_engineering");
        ...
        console.log(`[${this.name}] Current mode: ${this.modeService.getCurrentMode().name}`);
    }
    /**
     * Handle mode changes
     */
    handleModeChange(modeInfo) {
        console.log(`[${this.name}] Mode changed from ${modeInfo.oldMode || "none"} to ${modeInfo.newMode}`);
        this.applyModeSpecificSettings();
    }
    /**
    ...
    applyModeSpecificSettings() {
        ...
        const optimizations = this.modeService.getOptimizationsForService("prompt_engineering");
        ...
        if (currentMode.mode && MODE_TEMPLATES[currentMode.mode]) {
            ...
            console.log(`[${this.name}] Loaded ${Object.keys(MODE_TEMPLATES[currentMode.mode]).length} templates for ${currentMode.name}`);
        }
        console.log(`[${this.name}] Applied mode-specific settings for ${currentMode.name}`);
        ...
        switch (tool) {
            ...
            case "get_current_mode":
                return {
                    ...
                    metadata: {
                        optimizations: this.modeService.getOptimizationsForService("prompt_engineering"),
                    },
                };
            default:
                return {
     ...
     */
    async executeSummarizeText(id, parameters) {
        try {
            ...
            if (this.cachingStrategy !== "minimal" &&
                this.modeService.getCurrentMode().mode !== "testing") {
                // Cache implementation would go here
                this.verbose &&
                    console.log(`[${this.name}] Using caching strategy: ${this.cachingStrategy}`);
            }
     ...
     */
    async getSummary(prompt, maxLength) {
        try {
            const response = await axios_1.default.post(`${this.ollamaEndpoint}/api/generate`, {
...
exports.PromptEngineeringServer = PromptEngineeringServer;
/**
...
if (require.main === module) {
    ...
    const port = portArg !== -1 && portArg < args.length - 1
        ? parseInt(args[portArg + 1], 10)
        : parseInt(process.env.PROMPT_ENGINEERING_PORT || "3001", 10);
    const mode = modeArg !== -1 && modeArg < args.length - 1
        ? args[modeArg + 1]
        : process.env.DEVELOPMENT_MODE;
    const optimizeFor = optimizeForArg !== -1 && optimizeForArg < args.length - 1
        ? args[optimizeForArg + 1]
        : undefined;
    const verbose = verboseArg !== -1
...
//# sourceMappingURL=prompt-engineering-server.js.map