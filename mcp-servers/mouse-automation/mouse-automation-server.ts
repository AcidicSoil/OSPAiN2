...
interface MouseAutomationConfig {
...
export class MouseAutomationServer extends MCPServer {
  ...
  private acceleratedMode: boolean;
  ...
  constructor(config: MCPServerConfig & MouseAutomationConfig) {
    super(config);
    ...
    console.log(
      `[${this.name}] Screen size: ${this.screenSize.width}x${this.screenSize.height}`
    );
    ...
    if (this.allowedRegions.length > 0) {
      ...
      this.allowedRegions.forEach((region) => {
        console.log(
          `[${this.name}]   - ${region.name}: (${region.x1},${region.y1}) to (${region.x2},${region.y2})`
        );
      });
    }
  }
  ...
  private registerMouseClickTool(): void {
    const parameters: MCPToolParameter[] = [
      ...
      },
      {
        ...
        type: "number",
        description:
          "Delay in ms between clicks for double-click (overrides default)",
        required: false,
      },
    ];
   ...
   */
  private registerMouseMoveTool(): void {
    const parameters: MCPToolParameter[] = [
      {
        name: "x",
        type: "number",
   ...
   */
  private registerMouseDragTool(): void {
    const parameters: MCPToolParameter[] = [
      {
        name: "startX",
        type: "number",
   ...
   */
  private registerMouseSequenceTool(): void {
    const parameters: MCPToolParameter[] = [
      {
        name: "sequence",
        type: "array",
   ...
   */
  private registerScreenInfoTool(): void {
    const tool: MCPTool = {
      name: "screen_info",
      description:
        "Returns information about the screen size and mouse position",
      parameters: [],
      output: {
        type: "object",
  ...
  private async executeMouseClick(
    ...
    try {
      ...
      this.logOperation(
        "mouse_click",
        { x, y, button, double, delay: clickDelay },
        true
      );
  ...
  private async executeKeyboardPress(
    ...
    try {
      ...
      if (this.simulationMode) {
        if (text) {
          console.log(
            `[${this.name}] Simulating typing text: "${text}" with delay ${delay}ms`
          );
        } else if (modifier) {
          console.log(
  ...
  private async executeMouseDrag(
    ...
    try {
      ...
      this.logOperation(
        "mouse_drag",
        { startX, startY, endX, endY, button, steps },
        true
      );
   ...
   */
  private async executeMouseSequence(
    id: string,
    parameters: any
  ): Promise<MCPToolResponse> {
    try {
      ...
      for (let i = 0; i < sequence.length; i++) {
        ...
        try {
          ...
          switch (type) {
            case "click":
              result = await this.performMouseClick(
                params.x,
                params.y,
                params.button || "left",
                params.double || false,
                params.delay || this.clickDelay
              );
              break;
            case "move":
              result = await this.performMouseMove(
                params.x,
                params.y,
                params.speed || 5
              );
              break;
            case "drag":
              result = await this.performMouseDrag(
                params.startX,
                params.startY,
                params.endX,
                params.endY,
            ...
            case "delay":
              await new Promise((resolve) =>
                setTimeout(resolve, params.ms || 1000)
              );
              break;
            default:
   ...
   * Execute screen info
   */
  private async executeScreenInfo(
    id: string,
    parameters: any
  ): Promise<MCPToolResponse> {
    try {
  ...
  private async performMouseClick(
    ...
    if (this.simulationMode) {
      console.log(
        `[${this.name}] Simulating mouse click: x=${x}, y=${y}, button=${button}, double=${doubleClick}, delay=${delay}ms`
      );
      return;
    }
   ...
   */
  private async performMouseMove(
    x: number,
    y: number,
    speed: number
  ): Promise<void> {
    if (this.simulationMode) {
      console.log(
        `[${this.name}] Simulating mouse move: x=${x}, y=${y}, speed=${speed}`
      );
      return;
    }
  ...
  private async performMouseDrag(
    ...
    if (this.simulationMode) {
      console.log(
        `[${this.name}] Simulating mouse drag: from (${startX},${startY}) to (${endX},${endY}), button=${button}, steps=${steps}`
      );
      return;
    }
    ...
    if (!this.simulationMode) {
      try {
        const logFile = path.join(
          this.logDirectory,
          `mouse-automation-${new Date().toISOString().split("T")[0]}.log`
        );
