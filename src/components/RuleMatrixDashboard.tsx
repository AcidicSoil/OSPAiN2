...
interface RuleMatrixDashboardProps {
  rules: RuleFileInfo[];
  relationships: RuleRelationship[];
  onVisualize: (rules: RuleFileInfo[], relationships: RuleRelationship[], options: VisualizationOptions) => void;
  onExport: (format: 'html' | 'json' | 'svg') => void;
}
 ...
 */
const RuleMatrixDashboard: React.FC<RuleMatrixDashboardProps> = ({
  rules,
  relationships,
  onVisualize,
  onExport
  ...
  const [includedTypes, setIncludedTypes] = useState<RuleType[]>([]);
  const [excludedTypes, setExcludedTypes] = useState<RuleType[]>([]);
  ...
  const [availableTypes, setAvailableTypes] = useState<RuleType[]>([]);
  ...
  const handleGroupByChange = (event: React.ChangeEvent<HTMLSelectElement>) => {
    const value = event.target.value as VisualizationOptions['groupBy'];
  ...
  const handleTypeSelect = (event: React.ChangeEvent<HTMLSelectElement>) => {
    const selectedOptions = Array.from(event.target.selectedOptions).map(option => option.value) as RuleType[];
  ...
  const handleContentTypeSelect = (event: React.ChangeEvent<HTMLSelectElement>) => {
  ...
  const handleExport = (format: 'html' | 'json' | 'svg') => {
    onExport(format);
  };
  
  return (
    <div className="rule-matrix-dashboard">
      ...
      <div className="control-panel">
        <div className="control-section">
          ...
          <div className="control-group">
            ...
            <input 
              type="range" 
              id="minRelationshipStrength" 
              name="minRelationshipStrength"
              min="0" 
              max="1" 
              step="0.1" 
              value={options.minRelationshipStrength} 
          ...
          <div className="control-group">
            <label>
              <input 
                type="checkbox" 
                name="showOrphanedNodes" 
                checked={options.showOrphanedNodes} 
        ...
        <div className="control-section">
          ...
          <div className="control-group">
            <label>
              <input 
                type="checkbox" 
                name="showSemanticConnections" 
                checked={options.showSemanticConnections} 
                onChange={handleCheckboxChange}
              />
          ...
          <div className="control-group">
            ...
            <input 
              type="range" 
              id="minSemanticSimilarity" 
              name="minSemanticSimilarity"
              min="0" 
              max="1" 
              step="0.1" 
              value={options.minSemanticSimilarity} 
              onChange={handleSliderChange}
              disabled={!options.showSemanticConnections}
            />
          </div>
          ...
          <div className="control-group">
            <label>
              <input 
                ...
                checked={options.includeKeyPhrases} 
                onChange={handleCheckboxChange}
              />
              Include Key Phrases
            </label>
...
export default RuleMatrixDashboard; 