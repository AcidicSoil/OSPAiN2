...
Examples:
  ...
  command
    ...
    .action(async (title, options) => {
      try {
        ...
        let noteTags = options.tags ? options.tags.split(',').map((tag: string) => tag.trim()) : [];
        let noteCategory = options.category;

        if (options.ai) {
          // AI-assisted note creation
          console.log(chalk.blue('🧠 Generating note using AI...'));

          const note = await noteService.generateNoteWithAI(options.ai, title);

          if (note) {
            console.log(chalk.green('✅ Note created successfully with AI assistance!'));
            ...
            console.log(chalk.yellow('⚠️ AI generation failed. Falling back to manual entry.'));
          }
        }
        ...
        if (noteTags.length === 0) {
          ...
          noteTags = answers.tags ? answers.tags.split(',').map((tag: string) => tag.trim()) : [];
        }
        ...
        console.error(chalk.red('Error creating note:'), error);
      }
    });
    ...
    .action(async (options) => {
      try {
        ...
        displayedNotes.forEach((note, index) => {
          console.log(
            chalk.bold(`\n${index + 1}. ${note.title} `) +
              chalk.gray(`(ID: ${note.id.slice(0, 8)}...)`)
          );
          ...
          if (note.category) {
            console.log(chalk.magenta('   Category: ') + note.category);
          }
          ...
          const indicators = [];
          if (note.aiGenerated) indicators.push(chalk.blue('🧠 AI'));
          if (note.isDraft) indicators.push(chalk.yellow('📝 Draft'));
          if (note.relatedTodos && note.relatedTodos.length > 0) {
            indicators.push(chalk.green(`🔗 ${note.relatedTodos.length} todo(s)`));
          }
        ...
        console.log(
          chalk.bold(`\nStats: `) +
            chalk.gray(
              `${stats.totalItems} total, ${stats.aiGenerated} AI-generated, ${stats.drafts} drafts, ${stats.withTodoRelations} with todo links`
            )
        );
      } catch (error) {
  ...
  command
    ...
    .action(async (id, options) => {
      try {
        ...
        // Show indicators
        const indicators = [];
        if (note.aiGenerated) indicators.push(chalk.blue('🧠 AI Generated'));
        if (note.isDraft) indicators.push(chalk.yellow('📝 Draft'));
        if (note.sourceUrl) indicators.push(chalk.blue(`🔗 Source: ${note.sourceUrl}`));
        ...
        if (note.relatedTodos && note.relatedTodos.length > 0) {
          console.log(chalk.green(`🔗 Related Todos: ${note.relatedTodos.join(', ')}`));
        }

        console.log(chalk.gray(`Created: ${note.createdAt.toLocaleString()}`));
        console.log(chalk.gray(`Updated: ${note.updatedAt.toLocaleString()}`));
        ...
        if (options.open) {
          ...
          console.log(chalk.gray(`Opened in default markdown viewer: ${tempFilePath}`));
        }
      } catch (error) {
        console.error(chalk.red('Error viewing note:'), error);
      }
    });
    ...
    .action(async (id, options) => {
      try {
        ...
        if (
          ...
          // Show current note
          console.log(chalk.bold('Current note:'));
          displayNote(note);
          ...
          if (options.tags) updates.tags = options.tags.split(',').map((tag: string) => tag.trim());
          if (options.category !== undefined) updates.category = options.category || undefined;
          if (options.draft !== undefined) updates.isDraft = options.draft === 'true';
  ...
  command
    ...
    .action(async (id, options) => {
      try {
        ...
        if (!options.force) {
          console.log(chalk.bold('Note to delete:'));
          displayNote(note);
        ...
        if (deleted) {
          ...
          console.log(chalk.red('Failed to delete note.'));
        }
      } catch (error) {
    ...
    .action(async (options) => {
      try {
        ...
        if (!['md', 'json', 'html'].includes(format)) {
          console.log(chalk.red('Invalid format. Supported formats: md, json, html'));
          return;
        }
        ...
        // Export notes
        const exportedPath = noteService.exportNotes(
        ...
        if (exportedPath) {
          console.log(chalk.green(`✅ Notes exported successfully to: ${exportedPath}`));
        } else {
          console.log(chalk.red('Failed to export notes.'));
        }
      } catch (error) {
        console.error(chalk.red('Error exporting notes:'), error);
      }
    });
  ...
  command
    ...
    .action(async (noteId, todoIds) => {
      try {
        ...
        if (updatedNote) {
          console.log(chalk.green(`✅ Note linked to ${todoIds.length} todo(s)!`));

          if (updatedNote.relatedTodos) {
            console.log(chalk.cyan('Related todos: ') + updatedNote.relatedTodos.join(', '));
          }
        } else {
          console.log(chalk.red('Failed to link note to todos.'));
        }
      } catch (error) {
        console.error(chalk.red('Error linking note to todos:'), error);
      }
    });
  ...
  command
    ...
    .action(async (noteId, todoIds) => {
      try {
        ...
        if (updatedNote) {
          console.log(chalk.green(`✅ Note unlinked from ${todoIds.length} todo(s)!`));

          if (updatedNote.relatedTodos && updatedNote.relatedTodos.length > 0) {
            console.log(chalk.cyan('Still linked to: ') + updatedNote.relatedTodos.join(', '));
          } else {
            console.log(chalk.cyan('Note is not linked to any todos.'));
          }
        } else {
          console.log(chalk.red('Failed to unlink note from todos.'));
        }
      } catch (error) {
        console.error(chalk.red('Error unlinking note from todos:'), error);
      }
    });
  ...
  command
    ...
    .action(async (url, options) => {
      try {
        console.log(chalk.blue(`🌐 Creating note from webpage: ${url}`));

        const note = await noteService.createNoteFromWebPage(url);

        if (note) {
          console.log(chalk.green('✅ Note created successfully from web page!'));
          displayNote(note);
          ...
          console.log(chalk.red('Failed to create note from web page.'));
        }
      } catch (error) {
        console.error(chalk.red('Error creating note from web:'), error);
      }
    });
    ...
    .action(async (id, options) => {
      try {
        ...
        if (!['short', 'medium', 'long'].includes(length)) {
          console.log(chalk.red('Invalid length. Supported values: short, medium, long'));
          return;
        }

        console.log(chalk.blue('🧠 Generating summary...'));

        const summary = await noteService.summarizeNote(
          note.id,
        ...
        console.error(chalk.red('Error summarizing note:'), error);
      }
    });
  ...
  command
    ...
    .action(() => {
      try {
        ...
        console.log(chalk.cyan('Total Notes: ') + stats.totalItems);
        console.log(chalk.cyan('Drafts: ') + stats.drafts);
        console.log(chalk.cyan('AI Generated: ') + stats.aiGenerated);
        console.log(chalk.cyan('With Todo Links: ') + stats.withTodoRelations);
        ...
        console.error(chalk.red('Error getting note statistics:'), error);
      }
    });
 ...
 */
async function openInEditor(note: Note, noteService?: NoteService): Promise<void> {
  try {
    const tempFilePath = path.join(process.cwd(), 'tmp', `note-${note.id.slice(0, 8)}.md`);
    ...
    console.log(chalk.gray(`Opening in external editor: ${tempFilePath}`));

    const editor = process.env.EDITOR || 'code'; // Default to VS Code if EDITOR not set
    TerminalExecutor.execute(`${editor} "${tempFilePath}"`);

    // Wait for user to finish editing
    console.log(chalk.yellow('Waiting for you to finish editing...'));
    console.log(chalk.yellow('Press Enter when done to save changes.'));
    ...
    const category =
      categoryMatch && categoryMatch[1].trim() ? categoryMatch[1].trim() : note.category;
    ...
    if (noteService) {
      ...
      if (updatedNote) {
        console.log(chalk.green('✅ Note updated successfully with changes from editor!'));
      } else {
        console.log(chalk.red('Failed to update note with changes from editor.'));
      }
    }
  } catch (error) {
    console.error(chalk.red('Error opening note in external editor:'), error);
  }
}
 ...
 */
function resolveNoteId(id: string, noteService: NoteService): Note | null {
