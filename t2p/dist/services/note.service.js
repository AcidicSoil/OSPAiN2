...
const llm_middleware_service_1 = require("./llm-middleware.service");
/**
...
class NoteService {
     ...
     */
    constructor(notesFile = path.join(process.cwd(), 'master-notes.md'), llmService) {
        this.notesFile = notesFile;
        this.llmService = llmService || new llm_middleware_service_1.LLMMiddlewareService();
     ...
     */
    createNote(title, content, tags = [], category, relatedTodos, isDraft = false) {
        const now = new Date();
        const note = {
            id: (0, uuid_1.v4)(),
            title,
    ...
    getNotes(filter) {
        if (!filter)
            return [...this.notes.items];
        return this.notes.items.filter((note) => {
            // Apply each filter condition
            if (filter.tags && filter.tags.length > 0) {
                if (!note.tags.some((tag) => filter.tags.includes(tag)))
                    return false;
            }
            ...
            if (filter.hasTodoRelations !== undefined) {
                ...
                if (hasTodos !== filter.hasTodoRelations)
                    return false;
            }
     ...
     * @returns Updated note or null if not found
     */
    unlinkFromTodos(noteId, todoIds) {
        const note = this.getNote(noteId);
        if (!note || !note.relatedTodos)
            return null;
     ...
     */
    async generateNoteWithAI(prompt, title) {
        try {
      ...
      `;
            ...
            // Create the note
            return this.createNote(title || noteData.title, noteData.content, noteData.tags, noteData.category, undefined, false // Not a draft
            );
        }
        catch (error) {
     ...
     */
    async createNoteFromWebPage(url) {
        try {
            ...
            const result = terminal_executor_1.TerminalExecutor.execute(command);
            ...
            content += `\n\nSource: ${url}`;
            // Create the note
            const note = this.createNote(title, content, ['web-capture'], 'research', undefined, true // Mark as draft so user can review
            );
            // Save the source URL
     ...
     */
    exportNotes(format, outputPath, filter) {
        try {
