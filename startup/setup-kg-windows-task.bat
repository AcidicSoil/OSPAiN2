...
echo Knowledge Graph Windows Task Scheduler Setup
echo ===========================================
...
cd /d "%PROJECT_ROOT%"
set "PROJECT_ROOT=%cd%"
set "STARTUP_SCRIPT=%PROJECT_ROOT%\startup\knowledge-graph-startup.bat"
...
echo Creating scheduled task for Knowledge Graph server...

:: Create the task
schtasks /Create /F /TN "OSPAiN2\KnowledgeGraphStartup" ^
    /TR "'%STARTUP_SCRIPT%'" ^
    /SC ONLOGON ^
    /RU "%USERNAME%" ^
    /IT ^
...
if %errorlevel% equ 0 (
    ...
    echo Knowledge Graph server will start automatically on user login.
    echo.
    echo You can manage this task in the Windows Task Scheduler:
    echo 1. Search for "Task Scheduler" in the Start menu
    echo 2. Look for the "OSPAiN2\KnowledgeGraphStartup" task
    echo.
) else (
    echo.
