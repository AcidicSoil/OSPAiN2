 ...
 */
const ToolCallMonitor = ({ compact = false, showBypassButton = false, showResetTime = true, className = "", }) => {
    const [status, setStatus] = (0, react_1.useState)(RateLimitBypassService_1.default.getStatus());
    (0, react_1.useEffect)(() => {
        // Subscribe to status updates
        const unsubscribe = RateLimitBypassService_1.default.registerListener((newStatus) => {
            setStatus(newStatus);
        });
    ...
    const getStatusText = (status) => {
        if (status.bypassStatus === "active")
            return "Bypass Active";
        if (status.bypassStatus === "pending")
            return "Bypass Pending";
    ...
    const handleManualBypass = () => {
        RateLimitBypassService_1.default.executeBypass();
    };
    ...
    if (compact) {
        ...
        <div className="w-24 bg-gray-200 rounded-full h-2.5 dark:bg-gray-700 overflow-hidden">
          <div className={`h-2.5 rounded-full ${getColorClass(status.percentage)}`} style={{ width: `${status.percentage}%` }}></div>
        </div>
    ...
    return (<div className={`p-4 bg-white rounded-lg shadow dark:bg-gray-800 ${className}`}>
      <div className="flex justify-between items-center mb-2">
        ...
        {showResetTime && (<span className="text-sm text-gray-500 dark:text-gray-400">
      ...
      <div className="w-full bg-gray-200 rounded-full h-2.5 dark:bg-gray-700 mb-3">
        <div className={`h-2.5 rounded-full transition-all duration-500 ${getColorClass(status.percentage)}`} style={{ width: `${status.percentage}%` }}></div>
      </div>
      ...
      <div className="flex flex-col md:flex-row justify-between text-xs text-gray-500 dark:text-gray-400">
        ...
        {showBypassButton &&
            status.bypassStatus !== "active" &&
            status.bypassStatus !== "pending" && (<button onClick={handleManualBypass} className="mt-2 md:mt-0 px-3 py-1 text-xs font-medium text-center text-white bg-blue-600 rounded-lg hover:bg-blue-700 focus:ring-4 focus:outline-none focus:ring-blue-300 dark:bg-blue-500 dark:hover:bg-blue-600 dark:focus:ring-blue-800">
              Manual Bypass
            </button>)}
      </div>
    </div>);
};
exports.default = ToolCallMonitor;
//# sourceMappingURL=ToolCallMonitor.js.map