...
class RateLimitService {
  constructor() {
     ...
     */
    this.handleInterceptorMessage = (event) => {
  ...
  loadHistory() {
    try {
      ...
      const chunkCount = Number(localStorage.getItem('toolCallHistoryChunks') || '0');
      if (chunkCount > 0) {
        let history = [];
  ...
  calculateNextReset() {
    ...
    nextReset.setMinutes(nextReset.getMinutes() + this.settings.resetIntervalMinutes);
      ...
      if (prediction.willReachLimit) {
        this.rateLimitStatus.predictedTimeToLimit =
          prediction.estimatedTimeMinutes < 60
            ...
            : `~${Math.round(prediction.estimatedTimeMinutes / 60)}h ${Math.round(
                prediction.estimatedTimeMinutes % 60,
              )}m`;
      }
  ...
  predictRateLimitReach() {
    ...
    const remainingCalls = this.settings.toolCallLimit - this.rateLimitStatus.current;
    const estimatedTimeMinutes =
      callRate > 0 ? remainingCalls / callRate : this.settings.resetIntervalMinutes;
    return {
      willReachLimit: callRate > 0 && estimatedTimeMinutes < this.settings.resetIntervalMinutes,
  ...
  finalizeBypass(success) {
    if (success) {
      ...
      if (process.env.NODE_ENV === 'development') {
        this.rateLimitStatus.current = Math.floor(this.rateLimitStatus.current * 0.5);
        this.rateLimitStatus.percentage = Math.min(
          Math.round((this.rateLimitStatus.current / this.settings.toolCallLimit) * 100),
          100,
        );
      }
    } else {
  ...
  async performSessionRotation() {
    try {
      ...
      const sessionKeys = Object.keys(localStorage).filter(
        (key) => key.startsWith('cursor') || key.startsWith('ai_') || key.startsWith('tool_'),
      );
      console.log(`Would clear ${sessionKeys.length} session keys in a real implementation`);
  ...
  async performCacheOptimization() {
    try {
      ...
      const fileCalls = this.toolCallHistory.filter(
        (call) => call.tool === 'read_file' && call.parameters && call.parameters.target_file,
      );
      const fileFrequency = {};
      fileCalls.forEach((call) => {
        if (call.parameters?.target_file) {
          const file = call.parameters.target_file;
  ...
  processBatchedRequests() {
    ...
    console.log(`Processing ${this.batchedRequests.length} batched requests...`);
  ...
  executeCustomRules() {
    try {
      ...
      if (this.settings.customBypassRules && this.settings.customBypassRules.trim()) {
        // For safety, we're just logging the rules here
        // In a real implementation, we might use Function constructor with caution
        console.log('Custom rules to execute:', this.settings.customBypassRules);
      } else {
    ...
    try {
      ...
      if (typeof window !== 'undefined') {
        ...
        if (window.__cursor && window.__cursor.api) {
          console.log('Detected Cursor API - setting up real-time tool call monitoring');
          ...
          window.__cursor.api.callTool = (toolName, params) => {
   ...
   * Record a tool call
   */
  recordToolCall(tool, status = 'success', parameters) {
    // Guard against undefined or null tool
    if (!tool) {
      console.error('Attempted to record tool call with undefined or null tool name');
      return;
    }
    ...
    const newCall = {
      ...
      bypassMethod:
        this.rateLimitStatus.bypassStatus === 'active' ? this.settings.bypassMethod : undefined,
    };
    // Add to history
   ...
   */
  checkToolCallLimitWarning() {
    // If we're already at or beyond the action threshold and interceptor isn't active
    if (this.rateLimitStatus.current >= TOOL_CALL_ACTION_THRESHOLD && !this.isInterceptorActive) {
   ...
   */
  prepareToolCallInterceptor() {
    console.log('📢 Tool call limit approaching warning threshold. Preparing interceptor...');
    ...
    if (!this.externalBypassUrl) {
      ...
      this.externalBypassUrl = `${window.location.origin}/api/external-bypass/${Date.now()}`;
  ...
  activateToolCallInterceptor() {
    ...
    console.warn('🛑 CRITICAL: Tool call limit nearly reached. Activating interceptor...');
   ...
   */
  getHistory(limit) {
    return limit ? this.toolCallHistory.slice(0, limit) : [...this.toolCallHistory];
  }
  /**
   ...
   */
  notifyRealTimeListeners(record) {
